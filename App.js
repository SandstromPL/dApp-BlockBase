...
function App() {
  ...
  useEffect(() => {
    const checkRegistration = async () => {
      ...
      try {
        ...
        await loadTotalUsers(); // Load total users count // js task
      } catch (error) {
  ...
  // --- LOAD TOTAL USERS COUNT --- //js task
  const loadTotalUsers = async () => {
    if (!contract) return;
    try {
      const count = await contract.getTotalRegisteredUsers();
  ...
  return (
    <div className="App">
      <header className="App-header">
        {/*js task to display total number of users */}
        <div style={{
          position: 'absolute',
          top: '10px',
          right: '10px',
        ...
        {!isConnected ? (
          ...
          : !isRegistered ? (
            ...
            : (
              <div>
                <h2>
                  Welcome, {isEditing ? (  /*for edting the users name */ //solidity task */
                    <span>
                      <input
                        type="text"
                        value={newName}
